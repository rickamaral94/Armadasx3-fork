...
val EN: Map<String, String> = mapOf(
    ...
    "app.diag.profiler" to "Measure frame time (RSX Profiler)",
    "app.diag.profiler.desc" to "Writes a frame-time and stall report to the log every 300 frames, which is the only source of FPS data here. Costs a little performance, so leave it off when you are just playing. Takes effect immediately and resets to off on every launch.",
    "app.diag.blockSize" to "SPU Block Size",
    "app.diag.blockSize.desc" to "How much SPU code the recompiler compiles as one unit. Larger gives the optimiser more to work with and can make some blocks fail to compile, which drops that SPU thread to the interpreter. Applies on the next boot of a game.",
